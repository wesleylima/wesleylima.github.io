...
### Ease of Use
Gun people often explain the project like "a open source Firebase!"
 
No. It absolutely is not like firebase. The provided API is very obtuse and poorly documented.

Gun also has no real ACL layer. Their official soultion is to encrypt object using their "SEA" security library. Except that by doing that, you either have to use their very clunky user system, or roll your own access control in your application.
...
Yes, you can write your own key sharing mechanism using their security library library. But that's _you_ writing and ACL library. It's not Gun providing it.
...
Newcomers often ask: "Can gun do _x_?" And their answer is without exception always "Yes". Gun people do leave out the part where it's not actually Gun that's doing "x". It's you that are going to have to write that feature on top of Gun.
...
In fact, under the ideal "universal graph", a non-SEA node can't exist. But not only does documentation not explain this, all their examples are _fundamentally incompatible_ with how Gun would be used in the real world.

There is no "official" way to use Gun in the real world. Just some low level, buggy APIs for signing and encrypting. It's buggy now, but gun knows about and they're working on it! They've been working on this basic funcitonality for over 3 years. Yet they continue to imply Gun as fully featured and production-ready.
...
But usernames are not unique. So they they're of limited usefulness to a developer (or even a user). But yet again it's marketed without this caveat is completely glossed over.
...
Their mitigation for brute force attacks is "proof of work". What is not mentioned in the documentation is that the "proof of work" is only eforced when a server gets a request for a password. If a malicious server with your encrypted privated key wanted to, they are free to brute force the encrypted private key without providing a POW. _THIS IS NOT POINTED OUT DOCUMENTATION_

### Obsession With Fringe Features
The devloper is obsessed with micro-optimisation at the expense of everything else. In 2020, development essentially halted as the main developer (see #busfactor) worked on a more efficient "CPU Scheduler" to make it so that abnormally large JSON documents could be processed in a timely manner in a free Heroku instance.

The fact that this was prioritized over a massive slate of breaking bugs and missing features is mind boggling.

As a result, most of the 2020 "releases" were unstable. Which is of course extremely confusing for any developer when the "latest release" is actuallybuggy(er) than the a year-older release. Note that this instability wasn't noted anywhere in the documentation. You had to code along with Gun, find a bug, then go on their Discord and ask a question where (at least quickily) someone would tell you to roll back to last-year's releaase.
...
## The Community
I did mention the community in "The Good" section. Yes they're very friendly to newcommers, but the offical Gun people will _never_ discourage you from using Gun in your project--even if Gun is not a fit for most people at its current state.
...
This is very normal type of disclaimer for software at this stage and is _extremely_ informative to developers. Contrast Gun's exhuberant marketability implications with Orbit's disclaimer:
...
### The Code
The code is... It's mathemagical spaghetti. It's full of micro-optimizations that mostly serve to make the source code illegible. Gun's author explains this away as coding style or preference. He's approaching things like a Mathmatician! But even Mathematicians try to use common symbols for clarity!
...
The only part that of Gun is somewhat comprehensible is the SEA security library. This is probably due to the fact that it was in large part _not_ written by the main Gun author.

### Bus Factor=1

Speaking of code: while the Gun community is pretty vibrant, there aren't many contributors to the actual Gun code itself.

[insert contrib graphs here]
...
amark does the most contribution... and even more scary, he's probably one of the few people in the planet that actually understands the code.

### The Disease
Gun suffers from a (hopefully not terminal) case of NIH syndrome. Every problem in the world _must be solved by gun_ (See freeism).
...
Not only do I not think they can implement their ecosystem, I'm arguing they should not.
